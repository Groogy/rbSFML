module SFML
  ...
  class Rect
    ...
    #   Construct the rectangle from another rectangle, with the same coordinates.
    #   @param [Rect] rect Another rectangle
    # @overload initialize(ary)
    #   Construct the rectangle from a array.
    #   @param [Array] ary Array in the format <tt>[left, top, width, height]</tt>
    # @overload initialize(left, top, width, height)
    #   Construct the rectangle from its coordinates. Be careful, the last two parameters are the <tt>width</tt> and <tt>height</tt>, not the <tt>right</tt> and <tt>bottom</tt> coordinates!
    ...
    # @return [String] `Rect(left, top, width, height)`
    def inspect
    end
